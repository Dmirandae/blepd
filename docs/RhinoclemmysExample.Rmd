...
I conducted a phylogenetic diversity (PD) analysis for the Rhinoclemmys genus to evaluate the effect of branch length on the chosen conservation area(s). The topology used corresponds to a total evidence analysis from @alarcon2020a, and the distribution was modified from @le2008.
...
```
I found that results stabilized after approximately 1000 replicates, with the most substantial differences arising from using 10 or 100 replicates. As a general guideline, at least 100 replicates are necessary, but 1000 are preferable.

Next, I investigated whether the observed differences in results could be attributed to the length of the longest branches.
...
I found that branch length significantly influenced the selected area. While modifying internal branches had minimal effects, altering terminal branch lengths substantially impacted area selection. Specifically, a 46% decrease in _R. nasuta_'s or an 86% increase in _R. areolata_'s terminal branch length shifted the optimal area from Cho to Al. Similarly, a 59% increase in _R. rubida_'s or a 116% increase in _R. pulcherrima_'s terminal branch length resulted in a shift from Cho to Pl. 

As an additional exercise, I investigated the impact of ultrametricity. I hypothesized that if the tree were ultrametric, the terminal sister pairs would behave similarly, with differences primarily attributable to the underlying distribution. 
...
These findings suggest that decisions to conserve solely area Cho may be misguided. Areas Al, and potentially Pl, should also be considered as essential components of the conservation range.
