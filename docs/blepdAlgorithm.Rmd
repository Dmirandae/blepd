...
## new tree, with t1 branch chaged to "value" 
...
To evaluate the behaviour os a single termianl branch length we must:
...
And, the same result will arrive from changing the branch length of the terminal t1 from 1 and up, to find the upper limmit, the tie is solved in favour of area A2, opposite to the solution when we found the lower limit. In this case, even the smaller change in any terminal branch value, will modify the results.
